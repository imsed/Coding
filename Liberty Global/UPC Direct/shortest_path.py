...
def     isis_database_dict(isis_db_xml_file):
        ...
        return database
def     upc_direct_overlaping_nodes_style(node,overlap_nodes):
        if node in overlap_nodes:
                        return pydot.Node(node, style="filled", fillcolor="azure2", shape="red")
        else:
                        return pydot.Node(node, style="filled", fillcolor="azure2")
...
def     upc_direct_graph(file,direction,overlap_nodes):
        ...
        for h in nodes:
                ...
                s = upc_direct_overlaping_nodes_style(spoke,overlap_nodes)
                if graph_contain(s,G) is False:
                                G.add_node(s)
                color = "#%03x" % random.randint(0, 0xFFFFFF)
                for i in range(len(path)-1):
                        n = upc_direct_overlaping_nodes_style(path[i][0],overlap_nodes)
...
def     draw_graph(args):
        ...
        upc_direct_graph(args[0],args[1],args[2]).write(str(args[0]).split("hosts.txt")[0]+args[1]+"@"+st+".dot")
